...
Here is the whole loop, from a task to a merge:

{{< diagram >}}
- title: You specify the work
  kicker: GitHub issue · cloud task · local session
  desc: A paragraph is often enough. Well-scoped work goes to an agent; anything with real unknowns stays a local session where you're in the loop.
- title: An agent does it
  kicker: reads <code>CLAUDE.md</code> + <code>docs/memory/</code>
  edge: same committed rules, same memory
  desc: Local, cloud, and CI agents all follow the repo's contract and its accumulated fact files. Machine state carries nothing; the repo carries everything.
- title: You open the pull request
  kicker: from a prefilled "Create PR" link
  edge: agent pushes a <code>claude/</code> branch
  desc: The agent can't open the PR itself; it posts a prefilled link and you click it. The branch ruleset requires a PR for everything, so nothing reaches <code>main</code> unreviewed.
- title: A strong model reviews it
  kicker: automatic, once per PR
  edge: bugs + rule compliance
  desc: Every PR arrives pre-reviewed before you look at it, flagging the obvious and the subtle.
- title: You merge
  kicker: the one rule that never bends
  edge: human only
  desc: New learnings append to <code>docs/memory/</code> in the same PR, so the next agent starts ahead.
{{< /diagram >}}
...
**CI agents** live in GitHub Actions, in every repo. Two agent workflows,
plus a smoke test that proves they work:
...
- A **smoke test**. A third workflow files a trivial issue on demand and
  watches the full issue → branch → PR → review loop complete once. Proof
  the pipeline actually runs, not just that its checks are green.
...
packaged as plugins that load at session start. Skills update centrally
...
- **I merge more and type less.** My recurring interaction with most
  changes is reading a diff and clicking merge. The judgment work, what
  to build, whether it's right, whether it ships, stays mine.

## Why it became a factory

All of the above is roughly ten files of configuration per repository:
three workflows with the right permissions and model pins, settings
wiring for the plugin, the CLAUDE.md contract, the memory index, a
secret, an app installation, a branch ruleset. The first repo, you write
by hand. The second repo, you copy from the first, and now you have
drift. The third repo, the copies disagree in ways nobody notices,
because misconfigured agent pipelines don't fail loudly; they
...
work ships, the two agent workflows, branch protection, and the smoke
test. Live with it for a week; let the review gate and the memory
convention prove themselves. The factory step only makes sense when the
second and third repo make you copy files. When that day comes, you'll
want the platform treatment from the start:
